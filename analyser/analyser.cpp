...
namespace miniplc0 {
    ...
    std::optional<CompilationError> Analyser::analyseMain() {
        ...
        // <常量声明>
        auto const_err = analyseConstantDeclaration();
        if (const_err.has_value())
            return const_err;

        // <变量声明>
        auto var_err = analyseVariableDeclaration();
        if (var_err.has_value())
            return var_err;

        // <语句序列>
        auto state_err = analyseStatementSequence();
        if (state_err.has_value())
            return state_err;
    ...
    std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t &out) {
        ...
        } else if (token.GetType() == TokenType::PLUS_SIGN || token.GetType() == TokenType::MINUS_SIGN) {
            int prefix = token.GetType() == TokenType::PLUS_SIGN ? 1 : -1;
            next = nextToken();
            if (!next.has_value())
                return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrEOF);
