...
namespace miniplc0 {
    ...
    std::pair<std::optional<Token>, std::optional<CompilationError>> Tokenizer::nextToken() {
        ...
        // 关键词map
        static std::map<std::string, miniplc0::TokenType> keyword_map = {
        ...
        static std::map<char, miniplc0::TokenType> sign_map = {
        ...
        while (true) {
            ...
            switch (current_state) {
                ...
                case UNSIGNED_INTEGER_STATE: {
                    ...
                    for (; current_state == UNSIGNED_INTEGER_STATE; current_char = nextChar()) {
                        if (!current_char.has_value()) {
                            // EOF
                            return return_current_token();
                        }
                ...
                case IDENTIFIER_STATE: {
                    ...
                    //     如果解析结果是关键字，那么返回对应关键字的token，否则返回标识符的token
                    auto return_current_token = [&]() {
                        // LAMBDA FUNCTION: CONVERT STRINGSTREAM TO TOKEN (IDENT/ KW)
                ...
                case EQUAL_SIGN_STATE: {
                    ...
                    const auto &it = sign_map.find(ch);
                    if (it != sign_map.end()) {
                        return std::make_pair(std::make_optional<Token>(it->second, it->first, pos, currentPos()),
                                              std::optional<CompilationError>());
                    }
                    break;
                }
